...
@interface StepSlider ()
{
    ...
    CALayer* _sliderCircleTextLayer;
...
- (void)addLayers
{
    ...
    _sliderCircleTextLayer = [CALayer layer];
    _sliderCircleTextLayer.backgroundColor = [UIColor colorWithHex:@"#000000" alpha:0.85].CGColor;
    _sliderCircleTextLayer.cornerRadius = 6;
    _sliderCircleTextLayer.shadowColor = [UIColor colorWithHex:@"#000000" alpha:0.18].CGColor;
    _sliderCircleTextLayer.shadowOffset = CGSizeMake(0,0);
    _sliderCircleTextLayer.shadowRadius = 4;
    
    
    [self.layer addSublayer:_sliderCircleLayer];
    [self.layer addSublayer:_sliderCircleTextLayer];
    [self.layer addSublayer:_trackLayer];
    _labelFont = [UIFont systemFontOfSize:15.f];
    contentSize = self.bounds.size;
    [_sliderCircleTextLayer addSublayer:self.sliderCircleLabel];
}
...
- (CATextLayer *)sliderCircleLabel
{
    if (!_sliderCircleLabel) {
        
        _sliderCircleLabel = [CATextLayer layer];
//        _sliderCircleLabel.backgroundColor = [UIColor grayColor].CGColor;
        _sliderCircleLabel.contentsScale = [UIScreen mainScreen].scale;
        [_sliderCircleLabel setForegroundColor:[UIColor whiteColor].CGColor];
        [_sliderCircleLabel setFont:CFBridgingRetain([UIFont fontWithName:@"HelveticaNeue" size:17.0].fontName)];
        [_sliderCircleLabel setAlignmentMode:kCAAlignmentCenter];
        [_sliderCircleLabel setFontSize:17.0];
        _sliderCircleLabel.masksToBounds = YES;
        
        
    }
    
    return _sliderCircleLabel;
}

- (CATextLayer *)mintrackCircleLabel
{
    if (!_mintrackCircleLabel) {
        
        _mintrackCircleLabel = [CATextLayer layer];
        //字体的名字
        _mintrackCircleLabel.contentsScale = 2;
        //字体的大小
        _mintrackCircleLabel.fontSize = 14.f;
        //字体的对齐方式
        _mintrackCircleLabel.alignmentMode = kCAAlignmentLeft;
        //字体的颜色
        _mintrackCircleLabel.foregroundColor =
        [UIColor colorWithHex:@"#808080"].CGColor;
        _mintrackCircleLabel.string = @"5";
    }
    
    return _mintrackCircleLabel;
}

- (CATextLayer *)maxtrackCircleLabel
{
    if (!_maxtrackCircleLabel) {
        _maxtrackCircleLabel = [CATextLayer layer];
        //字体的名字
        _maxtrackCircleLabel.contentsScale = 2;
        //字体的大小
        _maxtrackCircleLabel.fontSize = 14.f;
        //字体的对齐方式
        _maxtrackCircleLabel.alignmentMode = kCAAlignmentRight;
        //字体的颜色
        _maxtrackCircleLabel.foregroundColor =
        [UIColor colorWithHex:@"#808080"].CGColor;
        _maxtrackCircleLabel.string = @"6";
    }
    
    return _maxtrackCircleLabel;
}
...
//            self.frame = newFrame;
    ...
    CGPoint oldTextPosition = _sliderCircleTextLayer.position;
    
    CAShapeLayer * trackLayerCopy = [NSKeyedUnarchiver unarchivedObjectOfClass:[CAShapeLayer class] fromData:[NSKeyedArchiver archivedDataWithRootObject:_trackLayer requiringSecureCoding:YES error:nil] error:nil];
//    CAShapeLayer * trackLayerCopy = [NSKeyedUnarchiver unarchiveObjectWithData:[NSKeyedArchiver archivedDataWithRootObject:_trackLayer]];
    ...
    if (self.sliderCircleImage) {
        ...
        _sliderCircleTextLayer.frame = CGRectMake(0, 0, 31, 34);
    ...
    _sliderCircleTextLayer.position = CGPointMake(contentFrame.origin.x + stepWidth * self.index, CGRectGetMidY(contentFrame) - 35);
    CGRect labelRect = [self.sliderCircleLabel.string boundingRectWithSize:_sliderCircleTextLayer.bounds.size options:NSStringDrawingUsesLineFragmentOrigin attributes:@{ NSFontAttributeName : [UIFont fontWithName:@"HelveticaNeue" size:17.0] } context:nil];
    self.sliderCircleLabel.frame = labelRect;
    self.sliderCircleLabel.position = CGPointMake(CGRectGetMidX(_sliderCircleTextLayer.bounds), CGRectGetMidY(_sliderCircleTextLayer.bounds));
    ...
    if (animated) {
        ...
        CABasicAnimation *basicSliderAnimation1 = [CABasicAnimation animationWithKeyPath:@"position"];
               basicSliderAnimation1.duration = [CATransaction animationDuration];
               basicSliderAnimation1.fromValue = [NSValue valueWithCGPoint:(oldTextPosition)];
        [_sliderCircleTextLayer addAnimation:basicSliderAnimation1 forKey:@"position"];
    }
    ...
    for (NSUInteger i = 0; i < self.maxCount; i++) {
        ...
        if (i == 0 || i == self.maxCount - 1) {
            trackCircle.bounds   = CGRectMake(0.f, 0.f, circleFrameSide + 5, circleFrameSide + 5);
            trackCircle.borderColor = [UIColor colorWithHex:@"#DADDE2"].CGColor;
            trackCircle.borderWidth = 1;
            trackCircle.cornerRadius = 4;
            trackCircle.masksToBounds = YES;
            
            if (i == 0) {
                self.mintrackCircleLabel.bounds   = CGRectMake(0.f, 0.f, 30,30);
                self.mintrackCircleLabel.position = CGPointMake(contentFrame.origin.x + stepWidth * i + 10, CGRectGetMidY(contentFrame) - 20);
                [self.layer addSublayer:self.mintrackCircleLabel];
            }else{
                self.maxtrackCircleLabel.bounds   = CGRectMake(0.f, 0.f, 30,30);
                self.maxtrackCircleLabel.position = CGPointMake(contentFrame.origin.x + stepWidth * i - 10, CGRectGetMidY(contentFrame) - 20);
                [self.layer addSublayer:self.maxtrackCircleLabel];
            }
            
        }else{
            trackCircle.bounds   = CGRectMake(0.f, 0.f, circleFrameSide, circleFrameSide);
            trackCircle.borderWidth = 0;
        }
        //        trackCircle.bounds   = CGRectMake(0.f, 0.f, circleFrameSide, circleFrameSide);
    ...
    [self.layer addSublayer:_sliderCircleLayer];
    [_sliderCircleTextLayer removeFromSuperlayer];
    [self.layer addSublayer:_sliderCircleTextLayer];
}
...
{
    return [self trackCircleIsSeleceted:trackCircle] ?[UIColor whiteColor].CGColor :[UIColor whiteColor].CGColor;
}
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    withoutCAAnimation(^{
        ...
        self->_sliderCircleTextLayer.position = CGPointMake(limitedPosition, self->_sliderCircleTextLayer.position.y);
