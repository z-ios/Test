...
@interface SingleSendItemView ()<UICollectionViewDelegate,UICollectionViewDataSource,WGCollectionViewLayoutDelegate>
...
@end
@implementation SingleSendItemView
...
- (void)setItemList:(NSArray<SingleSendItemModel *> *)itemList
{
    _itemList = itemList;
    
    [_itemList enumerateObjectsUsingBlock:^(SingleSendItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj.isSel) {
                self.selNum +=1;
            }
...
{
    SingleSendItemCollectionViewCell *cell= [collectionView dequeueReusableCellWithReuseIdentifier:@"SingleSendItemCollectionViewCell" forIndexPath:indexPath];
...
- (void)sureClick
{
    ...
    if (self.selSendDatas) {
        self.selSendDatas(arrM);
    }
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        [self.collectionView registerClass:[SingleSendItemCollectionViewCell class] forCellWithReuseIdentifier:@"SingleSendItemCollectionViewCell"];
