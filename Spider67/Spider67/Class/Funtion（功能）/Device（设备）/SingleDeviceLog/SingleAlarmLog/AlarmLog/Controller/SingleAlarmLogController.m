...
@interface SingleAlarmLogController ()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic, strong) BaseTableView* tableView;
@property (nonatomic, strong) NSMutableArray<SingleAlarmLogModel *>* alarmLogList;
...
@implementation SingleAlarmLogController
{
...
- (NSMutableArray<SingleAlarmLogModel *> *)alarmLogList
{
    if (!_alarmLogList) {
        _alarmLogList = [NSMutableArray array];
    }
...
- (void)setupUI
{
    ...
    [self.tableView registerClass:[SingleAlarmLogCell class] forCellReuseIdentifier:@"SingleAlarmLogCell"];
    // 设置回调（一旦进入刷新状态就会调用这个refreshingBlock）
    header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        //进行数据刷新操作
        [self screenDataAlarmArr:self.screenDict[@"alarmArr"] channelArr:self.screenDict[@"channelArr"] sdateStr:self.screenDict[@"sdateStr"] edateStr:self.screenDict[@"edateStr"]];
        
    }];
    self.tableView.mj_header = header;
    footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        [self dataAlarmArr:self.screenDict[@"alarmArr"] channelArr:self.screenDict[@"channelArr"] sdateStr:self.screenDict[@"sdateStr"] edateStr:self.screenDict[@"edateStr"]];
    }];
...
{
    SingleScreenController* screenVc = [SingleScreenController new];
...
{
    self.pageNum = 1;
    [self.alarmLogList removeAllObjects];
    [self dataAlarmArr:alarmArr channelArr:channelArr sdateStr:sdateStr edateStr:edateStr];
}

- (void)dataAlarmArr:(NSArray *)alarmArr channelArr:(NSArray *)channelArr sdateStr:(NSString *)sdateStr edateStr:(NSString *)edateStr
{
    self.screenDict[@"alarmArr"] = alarmArr;
    ...
    if (alarmArr.count >0) {
        [alarmArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            SingAlarmItemModel* model = obj;
            [alarms addObject:model.tagname];
        }];
    }
    ...
    if (channelArr.count > 0) {
        [channelArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [channels addObject:model.channelname];
        }];
    }
    ...
    [CenterNet.shared logAlarmParm:dict completion:^(NSArray * _Nullable dataList, BOOL success) {
        if (success) {
            NSArray* models = [NSArray yy_modelArrayWithClass:[SingleAlarmLogModel class] json:dataList];
            [self.alarmLogList addObjectsFromArray:models];
...
- (void)batchDeletionLog
{
    ...
    if (_alarmLogList.count > 0) {
        self.tableView.mj_header = nil;
        self.tableView.mj_footer = nil;
        [self.tableView setEditing:NO animated:YES];
        [self.tableView setEditing:YES animated:YES];
        _tableView.contentInset = UIEdgeInsetsMake(5, 0, 95, 0);
        [_alarmLogList enumerateObjectsUsingBlock:^(SingleAlarmLogModel*  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            obj.bgWith = 55;
...
- (void)delBtnClick
{
    ...
    UIAlertAction *alertT = [UIAlertAction actionWithTitle:NSLocalizedString(@"确定", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[self.tableView indexPathsForSelectedRows] enumerateObjectsUsingBlock:^(NSIndexPath * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [arr addObject:self.alarmLogList[obj.row].id];
        }];
        ...
        [CenterNet.shared delAlarmLogParm:dict completion:^(BOOL success) {
            [self.alarmLogList removeObjectsAtIndexes:insets];
...
{
    return _alarmLogList.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    SingleAlarmLogCell * cell = [tableView dequeueReusableCellWithIdentifier:@"SingleAlarmLogCell"];
    cell.model = _alarmLogList[indexPath.row];
    return cell;
}
...
- (void)didSelectItemIndexPath:(NSIndexPath *)indexPath
{
        ...
        UIAlertAction *alertT = [UIAlertAction actionWithTitle:NSLocalizedString(@"确定", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            NSDictionary* dict = @{
                @"idList": @[self.alarmLogList[indexPath.row].id]
            };
            
            [CenterNet.shared delAlarmLogParm:dict completion:^(BOOL success) {
                [self.alarmLogList removeObjectAtIndex:indexPath.row];
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        _tableView.contentInset = UIEdgeInsetsMake(5, 0, 15, 0);
