//
//  SingleAlarmShowSelCollectionView.m
//  Spider67
//
...
@interface SingleAlarmShowSelCollectionView ()<UICollectionViewDelegate,UICollectionViewDataSource,WGCollectionViewLayoutDelegate,DevicePtc>

@end
@implementation SingleAlarmShowSelCollectionView
...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(nonnull UICollectionViewLayout *)layout
{
    ...
    if (self) {
        ...
        [self registerClass:[SingleAlarmShowSelCollectionViewCell class] forCellWithReuseIdentifier:@"SingleAlarmShowSelCollectionViewCell"];
        [self registerClass:[SingAlermAddCollectionViewCell class] forCellWithReuseIdentifier:@"SingAlermAddCollectionViewCell"];
    }
    return self;
}

- (NSMutableArray<SingAlarmItemModel *> *)itemList
{
    if (!_itemList) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == _itemList.count) {
        ...
        SingleAlarmShowSelCollectionViewCell *cell= [collectionView dequeueReusableCellWithReuseIdentifier:@"SingleAlarmShowSelCollectionViewCell" forIndexPath:indexPath];
...
- (void)delAlarmItemIndexPath:(NSIndexPath *)indexPath
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self.itemList enumerateObjectsUsingBlock:^(SingAlarmItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if ([self.zdelegate respondsToSelector:@selector(selAlarmItemNum:)]) {
        [self.zdelegate selAlarmItemNum:[NSString stringWithFormat:@"%zd",_itemList.count]];
    }
}
