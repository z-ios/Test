...
static CGFloat XYMenuWidth = 176; // Menu宽度
static const CGFloat XYMenuItemHeight = 53; // item高度
...
#pragma mark --- 展示菜单
+ (void)showMenuWithImages:(NSArray *)imagesArr titles:(NSArray *)titles inView:(UIView *)view menuType:(XYMenuType)menuType clickHide:(BOOL)clickHide withItemClickIndex:(ItemClickIndexBlock)block
{
    ...
    [xy_menu showMenuWithImages:imagesArr titles:titles inView:view menuType:menuType clickHide:clickHide withItemClickIndex:block];
    
}

+ (void)showMenuWithImages:(NSArray *)imagesArr titles:(NSArray *)titles menuType:(XYMenuType)menuType currentNavVC:(UINavigationController *)currentNavVC clickHide:(BOOL)clickHide withItemClickIndex:(ItemClickIndexBlock)block
{
    if (menuType == XYMenuRightNavBar) {
        if ([[NSBundle currentLanguage] isEqualToString:@"zh-Hans"]) {
            XYMenuWidth = 160;
        }else{
            XYMenuWidth = 220;
    ...
    XYMenu *xy_menu = [[XYMenu alloc] init];
    [xy_menu showMenuWithImages:imagesArr titles:titles menuType:menuType currentNavVC:currentNavVC clickHide:clickHide withItemClickIndex:block];
}
...
- (void)showMenuWithImages:(NSArray *)imagesArr titles:(NSArray *)titles menuType:(XYMenuType)menuType currentNavVC:(UINavigationController *)currentNavVC clickHide:(BOOL)clickHide  withItemClickIndex:(ItemClickIndexBlock)block
{
    ...
    switch (menuType) {
        ...
        {
            self.menuInitRect = CGRectMake(16, statusHeight + navigationBarHeight - 12, 1, 1);
            self.menuResultRect = CGRectMake(16, statusHeight + navigationBarHeight - 12, XYMenuWidth, XYMenuHeight);
        }
            break;
    ...
    [self showAnimateMenuWithImages:imagesArr titles:titles menuType:menuType clickHide:clickHide withBlock:block];
...
- (void)showAnimateMenuWithImages:(NSArray *)imagesArr titles:(NSArray *)titles menuType:(XYMenuType)menuType clickHide:(BOOL)clickHide withBlock:(ItemClickIndexBlock)block
{
    __weak typeof(self) weakSelf = self;
    [self.menuView setImagesArr:imagesArr titles:titles withRect:self.menuResultRect withMenuType:menuType isDown:_isDown withItemClickBlock:^(NSInteger index, BOOL isSort) {
        if (clickHide) {
    ...
    [UIView animateWithDuration:0.35f delay:0.0f usingSpringWithDamping:0.85f initialSpringVelocity:0.0f options:UIViewAnimationOptionCurveEaseOut animations:^{
...
- (void)dismissXYMenu
{
    ...
    [self.menuView hideContentView];
    self.menuView.alpha = 1.0;
    [UIView animateWithDuration:0.5f delay:0.0f usingSpringWithDamping:1.0f initialSpringVelocity:0.0f options:UIViewAnimationOptionCurveEaseOut animations:^{
        self.menuView.frame = self.menuInitRect;
        self.menuView.alpha = 0.1;
    } completion:^(BOOL finished) {
        self.isDismiss = NO;
...
//    }
    ...
    switch (_menuType) {
        ...
        {
            if (_isDown) {
                self.menuInitRect = CGRectMake(midX+16, maxY, 1, 1);
                self.menuResultRect = CGRectMake(midX+16 - (XYMenuWidth / 4), maxY + 5, XYMenuWidth, XYMenuHeight);
            }else {
        ...
        case XYMenuRightNormal:
        {
            if (_isDown) {
                ...
                self.menuResultRect = CGRectMake(zScreenWidth - XYMenuWidth - 15, maxY + 5, XYMenuWidth, XYMenuHeight);
            }else {
                self.menuInitRect = CGRectMake(midX, minY, 1, 1);
