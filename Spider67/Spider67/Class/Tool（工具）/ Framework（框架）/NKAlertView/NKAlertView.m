...
- (void)show
{
    if (self.type == NKAlertViewTypeBottom) {
        ...
        [UIView animateWithDuration:0.4f delay:0.0f usingSpringWithDamping:1.0f initialSpringVelocity:0.0f options:UIViewAnimationOptionCurveEaseOut animations:^{
            self.alpha = 1.0;
...
- (void)setType:(NKAlertViewType)type
{
    ...
    if (self.contentView && _type == NKAlertViewTypeBottom) {
        ...
        UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:_contentView.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(0, 0)];
