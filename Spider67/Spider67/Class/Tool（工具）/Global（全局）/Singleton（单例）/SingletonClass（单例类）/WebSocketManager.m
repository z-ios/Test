...
-(void)connectServer{
    ...
    NSString* rrr = [[NSUserDefaults standardUserDefaults] objectForKey:@"serverIp"];
    NSString *str4;
    if ([rrr hasPrefix:@"https"]) {
        str4 = [rrr substringFromIndex:8];
    }else{
        str4 = [rrr substringFromIndex:7];
    }
    NSString* url = [NSString stringWithFormat:@"ws://%@/ws",str4];
    self.webScoket = [[SRWebSocket alloc] initWithURL:[NSURL URLWithString:url]];
...
-(void)webSocketDidOpen:(SRWebSocket *)webSocket{
    ...
    // 发送数据
    NSString * value = [NSString stringWithFormat:@"{\"messageType\":\"userId\",\"data\":{ \"userId\": \" %@ \"}}",CenterManager.shared.user_id];
...
-(void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error{
    NSLog(@"连接失败");
    [MBhud showText:@"WS连接失败" addView:zAppWindow];
    self.isConnect = NO;
...
-(void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message{
    NSLog(@"接收消息 ---- %@", message);
    NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:[message dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:nil];
    if ([dict[@"eventType"] isEqualToString:@"WS已连接"]) {
//        [MBhud showText:dict[@"eventType"] addView:zAppWindow];
    }else if ([dict[@"eventType"] isEqualToString:@"WS异常"]) {
        [MBhud showText:dict[@"eventType"] addView:zAppWindow];
    }
...
-(void)reConnectServer{
    ...
    //重连10次 2^10 = 1024
    NSInteger restNum = [[NSUserDefaults standardUserDefaults] integerForKey:@"websocketreset"];
    int cont = 1 << restNum;
    if (self.reConnectTime > cont) {
        self.reConnectTime = 0;
        return;
    }
...
-(void)senderheartBeat{
    ...
    dispatch_main_async_safe(^{
        if (ws.webScoket.readyState == SR_OPEN) {
            ...
            [MBhud showText:@"没网络，WS发送失败" addView:zAppWindow];
        }
    });
}
